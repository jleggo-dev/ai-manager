...
interface SchemaFieldDefExtended {
  description?: string;
  type?: string;
  required?: boolean;
...
interface FormattingStepResult {
  type: string;
  label?: string;
  changed?: boolean;
  rolledBack?: boolean;
...
interface LegacySchemaFieldDef {
  label: string;
  type: string;
  required: boolean;
  allowedValues: string[] | null;
...
function getJobConfig(job: ProcessingJob | null): ProcessingJobConfig {
...
interface ProcessingJobFormData {
  name: string;
  slug: string;
  description: string;
...
/* ══════════════════════════════════════════════════════════════
   BUILD RULES TAB — Single prompt editor + formatting rules
   ══════════════════════════════════════════════════════════════ */

/* ─── Helper: build a prompt-ready snippet for a single response field ─── */
function buildFieldSnippet(name: string, def: SchemaFieldDefExtended) {
...
/* ─── Helper: build the full schema as a prompt-ready JSON block ─── */
function buildFullSchemaSnippet(fields: Record<string, SchemaFieldDefExtended>) {
...
/* ─── Helper: copy text to clipboard with notification ─── */
...
/* ─── Inline sub-component: Collapsible Variables Reference with copy ─── */
function VariablesReference({ variables }: { variables: JobVariable[] }) {
...
/* ─── Inline sub-component: Response Schema Viewer (read-only + copy) ─── */
function ResponseSchemaViewer({ expectedSchema }: { expectedSchema: ExpectedSchema | null | undefined }) {
...
/* ─── Inline sub-component: Rule Set Schema Editor ─── */
function RuleSetSchemaEditor({
  schema,
  onChange,
}: {
