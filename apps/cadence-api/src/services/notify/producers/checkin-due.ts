 ...
 * affordance: `checkin_due` flips true at `generated_at + DEFAULT_HORIZON_DAYS`. This producer
 * reads the SAME fact the SAME way (see the candidate query's own comment for the bound), so the
 * push and the screen can never quietly disagree about which day it started being true.
 *
 * `target` is the date the week ran out — `generated_at` (a plan column that does not move while
 * the plan stays active) plus 7. That is the whole mechanism behind the brand requirement that a
...
async function produce(now: Date): Promise<NotifyRequest[]> {
  ...
  for (const row of await listCheckinDueCandidates()) {
    ...
    out.push({
      userId: row.user_id,
      kind: 'weekly_checkin',
      target: addDays(row.generated_at, 7),
