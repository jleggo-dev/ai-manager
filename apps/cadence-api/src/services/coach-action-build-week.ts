...
export const BUILD_NEXT_WEEK: CoachActionTool = {
  ...
  description:
    'Roll their SAME rhythm into next week once this week is OVER — a plain carry-forward, no rebuild: commitments recommitted unchanged; the app notifies them when ready. Takes effect immediately — the moment you call it next week is committed; no card, no tap. Use it only when they have plainly chosen to keep things as they are: "just build my week", a check-in ending with nothing to change, or an empty week they told you was fine. It never redesigns — a changed week is propose_plan_change (specific edits) or rebuild with scope "week". While this week is still running it refuses: next week written EARLY, check-in untouched, is shape_week with "build_ahead". With no plan yet, offer the build card. Afterwards say ONE short line that next week is on its way — never promise how long.',
