...
import { alwaysOnToolNames, onDemandToolNames, DOSSIER_FUNCTIONS } from './coach-tool-tiers.ts';
...
describe('what she carries', () => {
  it('offers every action on every turn — being chosen is what an action is', () => {
    const offered = new Set(coachToolDefinitions().map((d) => d.function.name));
    for (const a of coachActionNames()) expect(offered.has(a)).toBe(true);
  });
...
describe('find_tools', () => {
  it('finds a tool by a word the user would actually say', () => {
    expect(searchTools('workouts')).toContain('get_workout_history');
    expect(searchTools('journal')).toContain('get_journal');
    expect(searchTools('recipes')).toContain('get_recipes');
  });

  it('lists everything when asked for nothing in particular', () => {
    expect(searchTools('')).toEqual(onDemandToolNames());
    expect(searchTools('   ')).toEqual(onDemandToolNames());
  });

  /** A dead end is the one answer that leaves her stuck; the full list is always more useful. */
  it('falls back to the whole list rather than returning nothing', () => {
    expect(searchTools('xyzzy nonsense').length).toBeGreaterThan(0);
  });

  it('never offers a dossier fact or an action — those are already hers', () => {
    const tail = new Set(onDemandToolNames());
    for (const d of DOSSIER_FUNCTIONS) expect(tail.has(d)).toBe(false);
    for (const a of coachActionNames()) expect(tail.has(a)).toBe(false);
  });
...
describe('use_tool', () => {
  ...
  /** Actions carry their own consent contract; reaching one through here would bypass it. */
  it('will not run an action through the read door', async () => {
