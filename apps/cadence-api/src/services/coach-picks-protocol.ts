...
const CHECKIN_EDGE_RULES: string[] = [
  ...
  'WHAT THEY SAY ABOUT THE WEEK DECIDES THE NEXT CALL. If the week happened and only the logging is missing, build_next_week rolls it forward as a settled week. If something should change before next week is built, propose_plan_change carries the change. If a stretch of the week is not going to happen, a pause (shape_week, action "pause") or a detour holds it without deleting anything.',
];
