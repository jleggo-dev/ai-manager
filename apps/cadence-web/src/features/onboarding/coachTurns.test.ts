...
const LIST = { layout: 'list', multi: true, progress: 0.2, options: [{ label: 'Run a first 10k' }] };
const TILES = { layout: 'tiles', progress: 0.55, options: [{ label: '3', hint: 'most people keep this' }] };

describe('viewTurns', () => {
  it('strips the pick block out of what gets rendered', () => {
    const turns: CoachTurn[] = [{ role: 'coach', text: withPicks('What would you like to work on?', LIST) }];
...
describe('livePicks', () => {
  it("offers only the newest question's picks", () => {
    const views = viewTurns([
      { role: 'coach', text: withPicks('One?', LIST) },
      { role: 'user', text: 'run a first 10k' },
      { role: 'coach', text: withPicks('Two?', TILES) },
    ]);
    expect(livePicks(views, false)?.layout).toBe('tiles');
  });

  it('offers nothing while the turn is still streaming', () => {
    const views = viewTurns([{ role: 'coach', text: withPicks('One?', LIST) }]);
