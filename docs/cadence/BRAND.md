...
| Disruptions | `detours` (cause + duration) | **"My plan isn't working — I'm too busy" / Life happens. Let's take a detour.** (trigger leads with the felt statement in the user's voice — owner 2026-08-31; final copy + placement pending the design round) | cause is always named plainly (say "grief", never euphemize) |
