...
<<<<<<< HEAD
| **API-P2** 🟡 **Partial** (`refactor/api-p2-dead-code-metrics`) | Cadence API | **Done this slice:** deleted dead `services/dossier.ts`; removed unwired `services/completion.ts` (smoke script uses `applyRun`+`updateWear`; wire into `logOccurrence` still backlog); annotated `token-budget.ts` as not-yet-wired (kept + tested); unit-tested `rollingConsistency` (brand-critical); narrowed its param to `Pick<Occurrence,'date'\|'status'>` and dropped `as never` at progress/replan/plan-view call sites; removed unused DB-backed `consistency()` so `metrics.ts` stays pure/testable offline. **Still open:** `repos/occurrences.ts` slim return type / further `as never` cleanup elsewhere; test `retrieval/registry.ts` renderers; Zod at route + LLM-JSON boundaries; test `capture.ts`/`situation.ts` | Report 03 §3 (P2 sections) |
...
=======
| **API-P2** 🟡 **Partial** | Cadence API | **Done (#65 + phase3-api):** dead dossier/completion; `rollingConsistency` tests + type narrow; registry `render`/`rows` tests; `capture.ts`/`situation.ts` unit tests; Zod pilot (`validation/body.ts` on nutrition meals/targets); nomenclature `locked`→`committed` in registry + `tools/equipment`→`equipment` in coach-context. **Still open:** `repos/occurrences.ts` slim return type / further `as never` cleanup; expand Zod to remaining routes (plan/review/progress) | Report 03 §3 (P2 sections) |
| **WEB-P2** 🟡 **Partial** | Cadence web | **Done (phase3-web-*):** `NutritionTargets` extract + start-over phrase-gate tests; `useCoachChat` SSE-drop recovery hook; AuthScreen + App/`screenFromPlanStage` tests; dictation capability seam + MicButton; CSS Modules migration ADR. **Still open:** PlanView/ProgressView residual (paired WEB-04 largely done); full CSS Modules migration when styles.css ~800+ | Report 04 §3 (P2 section) |
...
| PKG-P2 ✅ **Done** (`refactor/phase3-pkg-aim-errors`) | Shared packages / Cadence AIM seam | Typed `AimError` boundary (`kind: config\|upstream\|not_found\|unknown`) wrapping `withAim`; coach + me routes branch on `httpStatus`/`kind` | Report 05 §4.5 |
>>>>>>> c700c41 (docs: FE-12 chat overlap ADR + WEB-P2 CSS plan; mark Phase 3 status)
...
=======
| **FE-12** ✅ **Done** (`docs/phase3-fe-12-chat-overlap`) | Research architectural overlap between AI Admin `TestChatPanel` streaming/session UI and Cadence web coach chat (SSE + session lifecycle) — decide whether a shared client helper is worth extracting later (research only; not a merge of the two UIs) | Frontend / Cadence | P3 | S (research) | Low | **Done** — ADR: do not extract shared package now; keep stacks separate (see `docs/cadence/adr/FE-12-chat-streaming-overlap.md`) |
>>>>>>> c700c41 (docs: FE-12 chat overlap ADR + WEB-P2 CSS plan; mark Phase 3 status)
