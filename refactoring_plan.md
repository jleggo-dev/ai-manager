...
| **CI-09** | `Devs.ai v2 resume error (400): Unrecognized key "tool_outputs"` surfaced during `e2e-devs-ai-v2-tools` debugging (PR #6) in the tool-fulfillment continuation call — doesn't currently fail the test (the flow recovers), but the resume request body shape for that endpoint looks wrong and deserves its own investigation | Backend | P2 | S-M | Low-Medium (live provider integration) | In Progress |
