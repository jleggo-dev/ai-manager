...
>   #72 DiagnosticsTab, #73 useHealthCheckProfilesData); STATUS_COLORS/`formatTimestamp` consolidation in flight (#82).
...
> - **CI-08** ✅ **Done** — `.gitattributes` `eol=lf` (PR #53; P3; §4.6)
> - **CI-09** ✅ **Done** — camelCase `toolOutputs` on Devs.ai v2 `/resume` (PR #55; §4.6)
> - **Phase 3 P2** — condensed area lists in §4.4 (batch 10 in flight; FE allowlist cleared)
...
| **FE-08** ✅ **Done** (`refactor/fe-08-use-health-check-profiles-data`, PR #20; fn-cap follow-up PR #73) | Extract a `useHealthCheckProfilesData` hook from `HealthCheckProfilesPage.tsx` (624 lines, no tests) | M | Low | **Done notes:** hook owns list CRUD, form state, key auto-resolve, and agent/model fetching (`hooks/useHealthCheckProfilesData.ts`); pure helpers in `lib/health-check-profiles.ts` (`filterEligibleProviders`, `filterKeysForProvider`, `buildAiOptions`, `buildModelOptions`, …); page is a thin render shell (~340 lines, dropped from eslint `max-lines` override). Tests: `health-check-profiles.test.ts` + page smoke (list/empty/create modal/delete). Structural only — no behavior changes beyond `aria-label` on edit/delete actions. **FE-P2 #73:** form/catalog/handlers split under `hooks/health-check-profiles/`; fn-cap allowlist entry removed. |
...
| **API-06** ✅ **Done** (`refactor/api-06-select-and-run`) | Extract shared `services/retrieval/select-and-run.ts` (`validateCalls`/`executeCalls`) from the duplicated `context-pack.ts`/`turn-context.ts` pipelines; add the resilience-contract test for `buildContextPack`'s 3-way fallback | M | Low | **Done notes:** `validateCalls`/`executeCalls` live in `retrieval/select-and-run.ts`; both pack-select and turn `context-select` pipelines consume them. Unit tests pin registry filtering + per-fn failure skip (`select-and-run.test.ts`). Resilience contract (`context-pack.test.ts`) covers `broker-curated` / `broker-partial` (select-ok+summarize-fail and reverse) / `deterministic`, empty/unknown select → deterministic, and mandatory `get_identity`+`get_constraints` append — AI/catalog/persist/registry mocked so CI needs no Cadence DB. Verify: 11/11 vitest; cadence-api `tsc`/`lint` clean. |
...
duplication (`apps/cadence-web`'s nutrition-day fetch was independently triplicated;
...
progress** (FE-11 · API-P2 · FE-P2 allowlist clear; **CI-06 Done** #60 — see §4.2). Remaining Phase 3
clusters stay available for later batches.
...
| **FE-P2** 🟡 **Partial** | Frontend | **Allowlist cleared (#69/#71/#72/#73):** `LovableGuidePage` → `pages/lovable-guide/*`; `HealthDashboardPage` → `pages/health-dashboard/*`; `DiagnosticsTab` → `organisms/diagnostics/*`; `useHealthCheckProfilesData` → `hooks/health-check-profiles/*` (fn-cap). Frontend eslint size-gate grandfather block removed. **STATUS_COLORS/`formatTimestamp` (#82 in flight).** **Still open (size-only densest):** `InvestigationPanel`, `WorkflowExecutionLog`, `FailoverConfigModal`, `WorkflowEditorPage`, `WorkflowTestSimulator`, `StepVariableMapper`, `ManageLlmsModal`, `HealthCheckConfigPage`, `WorkflowDetailPage`, `HealthCheckProvidersPage`, `WorkflowVariablePanel`, `WorkflowManager` | Report 02 §4 (4.11-4.23), §6 |
...
| PKG-P2 ✅ **Done** (`refactor/phase3-pkg-aim-errors`, PR #74) | Shared packages / Cadence AIM seam | Typed `AimError` boundary (`kind: config|upstream|not_found|unknown`) wrapping `withAim`; coach + me routes branch on `httpStatus`/`kind` | Report 05 §4.5 |
