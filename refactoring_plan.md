...
| **CI-07** | `@typescript-eslint/no-non-null-assertion` disabled repo-wide for `apps/cadence-api`/`apps/cadence-web` (both use `!` deliberately/extensively) and `react/no-unescaped-entities` disabled for `apps/cadence-web` (clashes with the brand's mandated contractions) — both are lint-suppressed rather than fixed; worth their own tickets if the team wants stricter enforcement later | Cadence | P3 | M (would require real fixes, not just re-enabling) | Low | **Partial** — `react/no-unescaped-entities` re-enabled as error in `apps/cadence-web` with call-site fixes (JSX string literals for brand contractions). **Remaining:** `@typescript-eslint/no-non-null-assertion` still off for cadence-api + cadence-web; no other packages in this slice |
